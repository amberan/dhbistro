...
if (isset($_REQUEST['logmein'])) { 
	...
	if ($logrec=mysqli_fetch_array ($logres)) {
		if ($logrec['sid']) { //new login
			...
			$_SESSION['sid'] = $sid; 
			debug("login new<");
		} else { //fallback loggedin odstranit po 1.5.3
...
function logout_forced($msg) {
	...
	mysqli_query ($database,"DELETE FROM ".DB_PREFIX."loggedin WHERE iduser=".$logrec['id']); //odstranit po 1.5.3
	session_destroy(); 
	session_start();
...
} else if (!isset($_SESSION['sid']) AND in_array(end(explode("/", $_SERVER['REQUEST_URI'])),$config['page_free']) == false) {
		logout_forced(null);
}
$_SESSION['timestamp'] = time();
