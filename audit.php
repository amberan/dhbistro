...
function linkType($type, $recid)
{
    if ($recid > 0) {
        switch ($type) {
            case 1: $link = 'readperson.php?rid='.$recid.'&hidenotes=0';
                break;
            case 2: $link = 'readgroup.php?rid='.$recid.'&hidenotes=0';
                break;
            case 3: $link = 'readcase.php?rid='.$recid.'&hidenotes=0';
                break;
            case 4: $link = '/reports/'.$recid.'';
                break;
            case 7: $link = 'readsymbol.php?rid='.$recid;
                break;
            case 8: $link = 'edituser.php?rid='.$recid;
                break;
            case 10: $link = 'tasks.php';
                break;
            default: break;
        }
...
switch ($filterCat) {
    case 0: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type NOT IN (5,11) ';
        break;
    case 1: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type<>11 ';
        break;
    case 2: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type=11 ';
        break;
    case 3: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type=1 ';
        break;
    case 4: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type=2 ';
        break;
    case 5: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type=3 ';
        break;
    case 6: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type=4 ';
        break;
    default: $filterSqlCat = ' WHERE '.DB_PREFIX.'audit_trail.record_type NOT IN (5,11) ';
}
switch ($filterType) {
    case 0: $filterSqlType = ' ';
        break;
    case 1: $filterSqlType = ' AND '.DB_PREFIX.'audit_trail.operation_type<>1 ';
        break;
    case 2: $filterSqlType = ' WHERE '.DB_PREFIX.'audit_trail.operation_type NOT IN (4,5,6,7,8,9) ';
        break;
...
';
	...
	<td>'.authorDB($rec['iduser']).'</td>
	<td>'.(($rec['time']) ? Date('d. m. Y (H:i:s)', $rec['time']) : 'nikdy').'</td>
	<td>'.$rec['ip'].'</td>
	<td>'.operationTypeDB($rec['operation_type']).'</td>
	<td>'.recordTypeDB($rec['record_type']).'</td>
	<td>';
        if ($rec['idrecord'] > 0) {
