...
if (isset($_POST['addsymbol2c'])) {
    ...
    $sql = "DELETE " . DB_PREFIX . "symbol2all
        FROM " . DB_PREFIX . "symbol2all
        JOIN " . DB_PREFIX . "case ON idrecord = id
        WHERE " . DB_PREFIX . "case.secret <= " . $user['aclSecret'] . " AND " . DB_PREFIX . "symbol2all.idsymbol=" . $_POST['symbolid'] . " AND " . DB_PREFIX . "symbol2all.table=3;";
    mysqli_query($database, $sql);
    if (isset($_POST['case'])) {
        $case = $_POST['case'];
    }
    mainMenu();
    sparklets('<a href="/symbols">symboly</a> &raquo; <a href="./editsymbol.php?rid='.$_POST['symbolid'].'">úprava symbolu</a> &raquo; <strong>uložení změn</strong>');
...
if (isset($_POST['addsymbol2ar'])) {
    ...
    $sql = "DELETE " . DB_PREFIX . "symbol2all
        FROM " . DB_PREFIX . "symbol2all
        JOIN " . DB_PREFIX . "report ON idrecord = reportId
        WHERE " . DB_PREFIX . "report.reportSecret <= " . $user['aclSecret'] . " AND " . DB_PREFIX . "symbol2all.idsymbol=" . $_POST['symbolid'] . " AND " . DB_PREFIX . "symbol2all.table=4;";
    mysqli_query($database, $sql);
    ...
    if (isset($_POST['report'])) {
        for ($i = 0; $i < Count($report); $i++) {
            $sqlinsert = "INSERT INTO " . DB_PREFIX . "symbol2all VALUES('" . $_POST['symbolid'] . "','" . $report[$i] . "','" . $user['userId'] . "','4')";
...
if (isset($_POST['addsymb2pers'])) {
    ...
    $sqlDescToNote = "INSERT INTO ".DB_PREFIX."note ( note, title, datum, iduser, idtable, iditem, secret, deleted) VALUES('".$rec_d['desc']."','Popis symbolu přiřazeného ".Date("j/m/Y H:i:s", Time())."','".Time()."','".$user['userId']."','1','".$_POST['person']."','0','0')";
    mysqli_query($database, $sqlDescToNote);
