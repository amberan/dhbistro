...
// NEW
if (isset($_POST['insertperson']) && !preg_match('/^[[:blank:]]*$/i', $_POST['name']) && !preg_match('/^[[:blank:]]*$/i', $_POST['contents'])
        && is_numeric($_POST['secret']) && is_numeric($_POST['side']) && is_numeric($_POST['power']) && is_numeric($_POST['spec'])) {
    if (is_uploaded_file($_FILES['portrait']['tmp_name'])) { //UPLOAD PORTRAIT
    ...
    if (is_uploaded_file($_FILES['symbol']['tmp_name'])) { //UPLOAD SYMBOL
        ...
        $sql_sy = "INSERT INTO ".DB_PREFIX."symbol
            ( symbol, `desc`, deleted, created, created_by, modified, modified_by, archived, assigned, search_lines, search_curves,
            search_points, search_geometricals, search_alphabets, search_specialchars, secret)
...
if (isset($_POST['personid'], $_POST['editperson']) && $user['aclPerson'] && !preg_match('/^[[:blank:]]*$/i', $_POST['name'])) {
    ...
    if (is_uploaded_file($_FILES['symbol']['tmp_name'])) { //UPLOAD SYMBOL
        $sqlCheckPersonalSymbol = "SELECT symbol, name, surname FROM " . DB_PREFIX . "person WHERE symbol>0 AND id=" . $_POST['personid']; //GET PERSON
        $sps = mysqli_query($database, $sqlCheckPersonalSymbol);
        if ($spc = mysqli_fetch_assoc($sps)) { //person already has symbol, unlinking
            // $prsn_res = mysqli_query($database, "SELECT name, surname FROM ".DB_PREFIX."person WHERE id=".$_POST['personid']); //GET PERSON
            // $prsn_rec = mysqli_fetch_assoc($prsn_res);
            $sdate = "<p>" . date("j/m/Y H:i:s", time()) . " Odpojeno od " . $spc['name'] . " " . $spc['surname'] . "</p>";
            $sqlUploadSymbol = "UPDATE " . DB_PREFIX . "symbol SET `desc` = concat('" . $sdate . "', `desc`), assigned=0 WHERE id=" . $spc['symbol']; //UPDATE SYMBOL
        ...
        $sql_sy = "INSERT INTO ".DB_PREFIX."symbol
            ...
            search_points, search_geometricals, search_alphabets, search_specialchars, secret)
            VALUES( '".$sfile."', '', 0, '".time()."', '".$user['userId']."', '".time()."', '".$user['userId']."', 0, 1, 0, 0, 0, 0, 0, 0, 0)";
        mysqli_query($database, $sql_sy); //INSERT SYMBOL
        $syidarray = mysqli_fetch_assoc(mysqli_query($database, "SELECT id FROM ".DB_PREFIX."symbol WHERE symbol = '".$sfile."'")); //SELECT SYMBOL
        $syid = $syidarray['id'];
        mysqli_query($database, "UPDATE ".DB_PREFIX."person SET symbol='".$syid."' WHERE id=".$_POST['personid']); //UPDATE PERSON
    }
    ...
    $update = "UPDATE ".DB_PREFIX."person SET name='".$_POST['name']."', surname='".$_POST['surname']."', phone='".$_POST['phone']."', ".$sqlPlayer."
        contents='".$_POST['contents']."', secret='".$_POST['secret']."', side='".$_POST['side']."', power='".$_POST['power']."', spec='".$_POST['spec']."'
         WHERE id=".$_POST['personid']; //UPDATE PERSON
