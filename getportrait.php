...
require_once (SERVER_ROOT.'/inc/func_main.php');
...
if (isset($_REQUEST['rid']) && is_numeric ($_REQUEST['rid'])) { //portret
    ...
    if ($getrec = mysqli_fetch_assoc ($getres)) {
        ...
        if (file_exists($config['folder_portrait'].$getrec['portrait']) and !is_dir(($_SERVER['DOCUMENT_ROOT'].$config['folder_portrait'].$getrec['portrait']))) {
    ...
    if (isset($_REQUEST['srid']) && is_numeric ($_REQUEST['srid'])) { //symbol
        $getres = mysqli_query ($database,"SELECT symbol FROM ".DB_PREFIX."person WHERE ".(($usrinfo['right_power']) ? '' : ' secret=0 AND ')." id=".$_REQUEST['srid']);
        if ($getrec = mysqli_fetch_assoc ($getres)) {
        ...
        if (isset($_REQUEST['nrid']) && is_numeric ($_REQUEST['nrid'])) { //symbol neprirazeny
            $getres = mysqli_query ($database,"SELECT symbol FROM ".DB_PREFIX."symbol WHERE id=".$_REQUEST['nrid']);
            if ($getrec = mysqli_fetch_assoc ($getres)) {
