...
if (isset($_GET['newsadd']) && $usrinfo['right_power']) { //ADD
	if ($_POST['insertnews'] && !preg_match ('/^[[:blank:]]*$/i',$_POST['nadpis']) && !preg_match ('/^[[:blank:]]*$/i',$_POST['obsah']) && is_numeric($_POST['kategorie'])) {
		mysqli_query ($database,"INSERT INTO ".DB_PREFIX."news (id, datum, iduser, kategorie, nadpis, obsah) VALUES('','".Time()."','".$usrinfo['id']."','".$_POST['kategorie']."','".$_POST['nadpis']."','".$_POST['obsah']."')");
		if (mysqli_affected_rows($database) == 1) {
			auditTrail(5, 3, 0);
