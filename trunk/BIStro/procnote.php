<?php
	...
	// úprava poznámky 
	if (isset($_POST['noteid']) && isset($_POST['editnote']) && !preg_match ('/^[[:blank:]]*$/i',$_POST['title']) && !preg_match ('/^[[:blank:]]*$/i',$_POST['note']) && is_numeric($_POST['secret'])) {
		...
		MySQL_Query ("UPDATE ".DB_PREFIX."notes SET title='".mysql_real_escape_string(safeInput($_POST['title']))."', datum='".Time()."', note='".mysql_real_escape_string($_POST['note'])."', secret='".$_POST['secret']."' WHERE id=".$_POST['noteid']);
